...
import (
	"AlgoCompress/lib/compression/vlc/table"
	"bytes"
	"encoding/binary"
	"encoding/gob"
	"log"
...
func New(tbGenerator table.Generator) EncoderDecoder {
	return EncoderDecoder{tblGenerator: tbGenerator}
}
...
func buildEncodedFile(tbl table.EncodingTable, data string) []byte {
	...
	buf.Write(splitByChunk(data, chunksSize).Bytes())
...
// it changes: ! + <lower case letter> -> to upper case latter.
//
